...
export default function FlagsPanel({ records = [] }: { records?: any[] }) {
  const { addToast } = useToast();
  const flagged = records.filter((r: any) => r.confidence < 80);
  ...
  return (
    <div className="rounded-2xl bg-white/[0.03] border border-white/[0.06] p-6">
      ...
      <div className="space-y-2.5 mb-4">
        {flagged.length === 0 && (
          <p className="text-xs text-white/30 py-4 text-center">No flagged items — all records above 80% confidence.</p>
        )}
        {flagged.map((item: any, idx: number) => {
          ...
          return (
            <div key={item.id || idx} className={`flex items-center gap-3 px-4 py-2.5 rounded-xl ${bgColor} border ${borderColor}`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <span className="text-[10px] text-white/30">{item.source || `Category: ${item.category}`}</span>
              </div>
              ...
              <button
                ...
                onClick={() => addToast(`Reviewing: "${item.description}" — ${item.category} (${item.confidence}% confidence)`, "info")}
              >
